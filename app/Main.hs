...
import Debug.Trace
import Arborist.Renamer (renameHaskell)
...
main = do
  ...
  let debugTreeStr = (debugTree . (.dynNode)) <$> renameHaskell haskell
  _ <- maybe (pure ()) putStrLn debugTreeStr
  withFile "myFile.txt" AppendMode $ \h -> do
        hSetEncoding h utf8  -- Set UTF-8 encoding
        hPutStrLn h (Text.unpack (pShowNoColor (head prgs)))
        hPutStrLn h (Text.unpack (pShowNoColor $ renameHaskell haskell))
