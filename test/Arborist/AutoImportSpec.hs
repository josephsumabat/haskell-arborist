...
spec = do
  describe "addDeclToImportEdit" $ do
    ...
    it "adds class with (..) to empty import list" $ do
      ...
      case importNode of
        Just import' -> do
          let changes = getChanges (addDeclToImportEdit import' classDecl)
          case changes of
    ...
    it "adds newtype with (..) to empty import list" $ do
      ...
      case importNode of
        Just import' -> do
          let changes = getChanges (addDeclToImportEdit import' newtypeDecl)
          case changes of
    ...
    it "adds data type with (..) to empty import list" $ do
      ...
      case importNode of
        Just import' -> do
          let changes = getChanges (addDeclToImportEdit import' dataDecl)
          case changes of
    ...
    it "adds regular function to empty import list" $ do
      ...
      case importNode of
        Just import' -> do
          let changes = getChanges (addDeclToImportEdit import' fooDecl)
          case changes of
    ...
    it "adds operator with parentheses to empty import list" $ do
      ...
      case importNode of
        Just import' -> do
          let changes = getChanges (addDeclToImportEdit import' operatorDecl)
          case changes of
